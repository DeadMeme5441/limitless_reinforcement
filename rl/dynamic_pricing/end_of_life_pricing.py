...
class ClearancePricingMDP:
    ...
    def get_vf_for_policy(
        ...
        mrp: FiniteMarkovRewardProcess[WithTime[int]] = self.mdp.apply_finite_policy(
            policy
        )
        return evaluate(unwrap_finite_horizon_MRP(mrp), 1.0)

    def get_optimal_vf_and_policy(
        self,
    ) -> Iterator[Tuple[V[int], FiniteDeterministicPolicy[int, int]]]:
        return optimal_vf_and_policy(unwrap_finite_horizon_MDP(self.mdp), 1.0)
...
if __name__ == "__main__":
    ...
    cp: ClearancePricingMDP = ClearancePricingMDP(
        initial_inventory=ii, time_steps=steps, price_lambda_pairs=pairs
    )
    ...
    stationary_policy: FiniteDeterministicPolicy[int, int] = FiniteDeterministicPolicy(
    ...
    vf_for_policy: Iterator[V[int]] = evaluate(
        unwrap_finite_horizon_MRP(finite_horizon_MRP(single_step_mrp, steps)), 1.0
    )
    ...
    for t, (vf, policy) in enumerate(cp.get_optimal_vf_and_policy()):
        ...
        pprint(vf)
        print(policy)
        prices.append([pairs[policy.action_for[s]][0] for s in range(ii + 1)])
