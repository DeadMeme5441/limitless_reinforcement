...
class InventoryDeterministicPolicy(DeterministicPolicy[InventoryState, int]):
...
class InventoryStochasticPolicy(Policy[InventoryState, int]):
    ...
    def act(self, state: NonTerminal[InventoryState]) -> SampledDistribution[int]:
        def action_func(state=state) -> int:
            reorder_point_sample: int = np.random.poisson(
...
@dataclass(frozen=True)
class InventoryMDPNoCap(MarkovDecisionProcess[InventoryState, int]):
    poisson_lambda: float
    holding_cost: float
    stockout_cost: float

    def step(
        self, state: NonTerminal[InventoryState], order: int
    ...
    def fraction_of_days_oos(
        self, policy: Policy[InventoryState, int], time_steps: int, num_traces: int
    ) -> float:
        impl_mrp: MarkovRewardProcess[InventoryState] = self.apply_policy(policy)
        count: int = 0
        ...
        for _ in range(num_traces):
            steps = itertools.islice(
                impl_mrp.simulate_reward(Constant(NonTerminal(start))), time_steps
            )
            for step in steps:
...
if __name__ == "__main__":
    ...
    si_mdp_nocap = InventoryMDPNoCap(
        poisson_lambda=user_poisson_lambda,
        holding_cost=user_holding_cost,
        stockout_cost=user_stockout_cost,
    )

    si_dp = InventoryDeterministicPolicy(reorder_point=user_reorder_point)

    oos_frac_dp = si_mdp_nocap.fraction_of_days_oos(
        policy=si_dp, time_steps=user_time_steps, num_traces=user_num_traces
    )
    print(
