...
def finite_horizon_MRP(
    process: FiniteMarkovRewardProcess[S], limit: int
...
def unwrap_finite_horizon_MRP(
    ...
    def single_without_time(
        ...
        if isinstance(s_r[0], NonTerminal):
            ret: Tuple[State[S], float] = (NonTerminal(s_r[0].state.state), s_r[1])
        else:
    ...
    return [
        ...
        for _, states in groupby(
            sorted((nt.state for nt in process.non_terminal_states), key=time), key=time
        )
    ]


def evaluate(steps: Sequence[RewardTransition[S]], gamma: float) -> Iterator[V[S]]:
        ...
        v.append(
            {
                s: res.expectation(
                    lambda s_r: s_r[1]
                    + gamma * (extended_vf(v[-1], s_r[0]) if len(v) > 0 else 0.0)
                )
...
def finite_horizon_MDP(
    process: FiniteMarkovDecisionProcess[S, A], limit: int
    ...
    """
    mapping: Dict[
        WithTime[S], Dict[A, FiniteDistribution[Tuple[WithTime[S], float]]]
    ] = {}
...
def unwrap_finite_horizon_MDP(
    ...
    return [
        {
            NonTerminal(s.state): without_time(process.mapping[NonTerminal(s)])
            for s in states
        }
        for _, states in groupby(
            sorted((nt.state for nt in process.non_terminal_states), key=time), key=time
            ...
            action_values = (
                (
                    res.expectation(
                        lambda s_r: s_r[1]
                        + gamma
                        * (extended_vf(v_p[-1][0], s_r[0]) if len(v_p) > 0 else 0.0)
                    ),
                    a,
                )
